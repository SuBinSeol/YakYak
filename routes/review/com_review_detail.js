...
router.get('/', function(req, res, next){
    ...
    var sql3 = 'SELECT COUNT(*) AS count FROM review_comment where review_no=?';
    console.log(sql3);
    p=parseInt(n.num);
    conn.query(sql3, p, function(err, rows, fields) {
        // console.log('대앳글수: ', rows[0].count);
        count2 = rows[0].count;
    });
});
...
router.post('/', function (req, res) {
    ...
    var paramss = [parseInt(count2)+1, parseInt(n.num) ];
    console.log("으아아아아"+parseInt(count));
    conn.query(sql, params, function(err) {
        if(err) console.log('query is not excuted. insert fail...\n' + err);
        else {
            console.log("댓글 작성 완료");
            conn.query(sqls, paramss, function(err, row) {
                if(err) console.log('query is not excuted. insert fail...\n' + err);
                else {
                    console.log("댓글수="+row);
